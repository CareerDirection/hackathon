...
{id:"Q300",marks:4,text:"Which DS is used in LRU cache implementation?",options:["A) Stack","B) Queue","C) HashMap + Doubly Linked List","D) Array"],correct:"C"},
  // 👉 ADD REST OF YOUR QUESTIONS HERE
];
...
function renderCurrentSet(){
  ...
  const start = currentSetIndex*10;
  const end = start + 10;
  const set = chosenQuestions.slice(start, end);

  set.forEach((q, i)=>{
    const div = document.createElement("div");
    div.className = "questionBlock";
    div.innerHTML = `<p><b>Q${start+i+1}. ${q.text}</b></p>`;
  ...
  $("nextSetBtn").textContent = currentSetIndex < 2 ? "Next Page →" : "Submit Hackathon";
}
...
function startTimer(){
  timerId = setInterval(()=>{
    ...
    if(remainingSeconds<=0) finishQuiz("time");
  },1000);
}
...
function issueWarning(reason){
  ...
  showToast(`Warning ${warnings}/${MAX_WARNINGS}: ${reason}`);

  if(warnings>=MAX_WARNINGS) finishQuiz("violation");
}
...
document.addEventListener("visibilitychange",()=>{
  if(document.hidden) issueWarning("Tab switch detected");
});
window.addEventListener("blur",()=>{
  issueWarning("Window lost focus");
});
...
function computeResult(){
  ...
  chosenQuestions.forEach(q=>{
    ...
    if(!sel) unansweredCount++;
    else if(sel.value===q.correct){
      correctMarks += q.marks;
      correctCount++;
    } else wrongCount++;
  });

  const wrongPenalty = wrongCount * WRONG_PENALTY;
  const unPenalty = Math.floor(unansweredCount/5) * UNANSWERED_BLOCK_PENALTY;

  let finalScore = correctMarks - wrongPenalty - unPenalty;
  if(finalScore<0) finalScore=0;

  return {correctMarks, correctCount, wrongCount, unansweredCount, wrongPenalty, unPenalty, finalScore};
}
...
function finishQuiz(reason){
  ...
  $("finalScore").textContent = `Your Score: ${r.finalScore} / 100`;

  $("resultBreakdown").innerHTML = `
    <div class="violationTitle">Result Breakdown</div>
    <ul class="violationList">
      <li>Correct Answers: ${r.correctCount} (+${r.correctMarks})</li>
      <li>Wrong Answers: ${r.wrongCount} (-${r.wrongPenalty})</li>
      <li>Unanswered: ${r.unansweredCount} (-${r.unPenalty})</li>
    </ul>`;
...
document.addEventListener("DOMContentLoaded",()=>{
  $("startBtn").addEventListener("click",()=>{
    ...
    if(!name||!email) return alert("Fill all details");

    if(isLocked(email)){
      showSection("resultSection");
      return alert("You already attempted. Try after 20 days.");
    }
  ...
  $("nextSetBtn").addEventListener("click",()=>{
    if(currentSetIndex<2){currentSetIndex++;renderCurrentSet();}
    else finishQuiz("normal");
  });
