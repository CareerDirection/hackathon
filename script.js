...
// ================= QUESTION BANK =================
// 🔴 ADD MORE QUESTIONS BELOW Q2 LATER
const questionBank = [
...
function pickQuestions() {
  ...
  shuffle(copy);
  chosenQuestions = copy.slice(0, Math.min(30, copy.length));
}
...
function startTimer() {
  ...
  timerInterval = setInterval(() => {
    ...
    if (remainingSeconds <= 300) timerEl.classList.add("timerDanger");
...
function finishQuiz() {
  ...
  $("resultBreakdown").innerHTML = `
    ...
    <div class="violationNote">
      Final Score = Correct Marks − Negative Marks
    </div>
  `;

  $("resultMessage").textContent =
    r.finalScore >= 70 ? "🎉 Excellent Performance!" :
    r.finalScore >= 40 ? "👍 Good Attempt!" :
    "📘 Keep Practicing!";
