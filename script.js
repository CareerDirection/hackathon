...
const MAX_WARNINGS = 10;           // 10 chances
const LOCK_DAYS = 20;
...
function issueWarning(reason) {
  ...
  if (warningEl) {
    warningEl.textContent = warnings.toString();
    // Reset classes
    ...
    // 0–3 Green, 4–6 Orange, 7–10 Red + blink
    if (warnings <= 3) {
      warningEl.classList.add("warningGreen");
    } else if (warnings <= 6) {
      warningEl.classList.add("warningOrange");
    } else {
      warningEl.classList.add("warningRed", "warningBlink");
    }
  }
  ...
  // Category based styling for toast
  let theme = "default";
  if (reason.includes("Tab") || reason.includes("Window")) theme = "tab";
  else if (reason.toLowerCase().includes("sound") || reason.toLowerCase().includes("mic")) theme = "audio";
  else if (reason.includes("Camera")) theme = "camera";
  else if (
    reason.includes("Copy") ||
    reason.includes("Paste") ||
    reason.toLowerCase().includes("shortcut")
  )
    theme = "keyboard";
