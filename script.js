...
// ================== QUESTION BANK ==================
// 5 × 3 MARKS + 5 × 4 MARKS for now.
// You can ADD MORE questions later in the SAME format.
// marks: 3 or 4
// correct: "A" | "B" | "C" | "D"
...
const questionBank = [
  ...
  // ➜ You can add more questions here in same structure later.
];

// ================== GLOBAL STATE ==================
let chosenQuestions = [];
let currentSetIndex = 0;        // 0-based set index
let userAnswers = {};
...
// Simple audio monitor: Loud sound > 3 seconds → warning
    ...
    audioMonitorId = setInterval(() => {
      ...
      if (avg > THRESHOLD) {
        ...
        if (now - loudStartTime >= REQUIRED_MS) {
          issueWarning("Continuous loud sound / speaking detected via mic.");
          loudStartTime = now;
        }
      } else {
        loudStartTime = null;
    ...
    brightnessCheckId = setInterval(() => {
      ...
      if (avgBrightness < BRIGHTNESS_THRESHOLD) {
        ...
        if (lowBrightnessCount >= REQUIRED_CHECKS) {
          issueWarning("Camera is too dark or covered. Keep your face visible.");
...
// Visibility / blur = tab change / window change
...
// Block copy / paste / right-click / selection / shortcuts
...
function pickQuestionsForExam() {
  // If you want strict 20×3M + 10×4M when you add more, you can
  // modify this function later. For now we use ALL existing questions
  // (which is 10).
  chosenQuestions = [...questionBank];
  shuffleArray(chosenQuestions);

  const totalQ = chosenQuestions.length;
  setsCount = Math.min(3, Math.ceil(totalQ / 10));
}
...
function computeResult() {
  ...
  const wrongPenalty = wrongCount * 2; // –2 each wrong
  const unansweredPenalty = Math.floor(unansweredCount / 3) * 2; // –2 per 3 unanswered
...
function showSection(id) {
  ["loginSection", "examSection", "resultSection"].forEach((secId) => {
    const el = $(secId);
...
function showResult(score, autoSubmitted, reasonCode) {
  ...
  if (currentUser && resultStudent) {
    resultStudent.textContent =
      ...
      `College: ${currentUser.college} | Stream: ${currentUser.stream}`;
  }
  ...
  if (autoSubmitted && reasonCode === "violation") {
    // No badge on violation — show violation reasons instead
    if (resultBadgeBox) {
      let html =
        `<p style="color:#b20000; font-weight:bold;">Your session was auto-submitted due to security violations (camera/mic/noise/tab switching or restricted actions).</p>`;
      ...
      html += `<p>If you are serious, you can request 1 re-attempt:<br>
      1️⃣ Take a screenshot of this page<br>
      2️⃣ Email support team with justification.<br><br>
      Note: Another violation may block your access for 3 months.</p>`;
    ...
    // Normal completion — show badge
    if (resultBadgeBox) {
      resultBadgeBox.innerHTML =
        `<img src="badge.png" alt="Hackathon Badge">`;
    }
    if (score >= 90) {
      msg =
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // If previous result exists & lock active → show Result page
  ...
  if (startBtn) {
    startBtn.addEventListener("click", () => {
      ...
      if (isLocked(email)) {
        ...
        // If we have stored result, show it
        const lastEmail = localStorage.getItem("hack_last_email");
        const lastResultStr = localStorage.getItem("hack_last_result");
