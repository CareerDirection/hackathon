...
if(/Mobi|Android|iPhone/i.test(navigator.userAgent)){
 alert("Hackathon works only on Laptop/Desktop.");
...
function beep(){
 ...
 osc.frequency.value=900; gain.gain.value=0.2;
...
function finishExam(){
 ...
 // 🎯 SCORE BASED PROFESSIONAL MESSAGE
 let message="";
 if(r.final>=75){
  message="🌟 Outstanding performance! You have qualified for the Winner Category and are eligible for Internship Opportunities.";
 }
 else if(r.final>=50){
  message="🎉 Great job! You are eligible for Internship Participation.";
 }
 else if(r.final>=30){
  message="👍 Good attempt! Keep improving and participate again to unlock more opportunities.";
 }
 else{
  message="👏 Thank you for participating in the Hackathon. Keep learning and try again in the next challenge!";
 }

 $("resultMessage").textContent=message;

 $("resultStudent").textContent=
  `Name: ${currentUser.name} | Email: ${currentUser.email} | College: ${currentUser.college}`;
...
document.addEventListener("DOMContentLoaded",()=>{
 $("startBtn").onclick=()=>{
  ...
  const streamVal=$("stream").value.trim();

  if(!nameVal||!emailVal||!collegeVal||!streamVal){
   alert("Fill all fields");
   return;
  }

  if(isLocked(emailVal)){
   alert("You already attempted. Try again after 15 days.");
   return;
  }

  currentUser={name:nameVal,email:emailVal,college:collegeVal,stream:streamVal};
  startExam();
 };
