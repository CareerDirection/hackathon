...
// Window unfocus
window.addEventListener("blur",()=> addWarning("Window focus lost"));
...
function finishQuiz(){
  ...
  localStorage.setItem("examAttemptTime", Date.now());
...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  $("startBtn").onclick=()=>{
    const lastAttempt=localStorage.getItem("examAttemptTime");
    if(lastAttempt && Date.now()-lastAttempt < LOCK_TIME){
      alert("You can attempt again after 20 days.");
      return;
    }
  ...
  $("adminResetBtn").onclick=()=>{
    if($("resetCode").value===ADMIN_RESET_CODE){
      localStorage.removeItem("examAttemptTime");
