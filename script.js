...
if(/Mobi|Android|iPhone/i.test(navigator.userAgent)){
 document.body.innerHTML="<h2 style='text-align:center;margin-top:50px'>Laptop/Desktop Only</h2>";
}
...
function handleCopy(e){ if(inExam&&!quizFinished){ e.preventDefault(); issueWarning("Copy attempt detected"); }}
function handleRightClick(e){ if(inExam&&!quizFinished){ e.preventDefault(); issueWarning("Right-click attempt detected"); }}
function handleSelection(){
 if(inExam&&!quizFinished){
   const sel=window.getSelection().toString();
   if(sel.length>0){ issueWarning("Text selection detected"); window.getSelection().removeAllRanges(); }
 }
}
...
// ================= START =================
function startExam(){
 inExam=true; warnings=0; userAnswers={}; remainingSeconds=EXAM_DURATION_MIN*60;
