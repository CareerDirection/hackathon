...
function setLock(email){
 const expireTime = Date.now() + LOCK_DAYS*24*60*60*1000;
 localStorage.setItem(lockKey(email), expireTime);
}
...
function finishExam(){
 ...
 setLock(currentUser.email); // 🔒 LOCK USER
...
document.addEventListener("DOMContentLoaded",()=>{
 $("startBtn").onclick=()=>{
  initAudio(); // 🔊 unlock audio on user click
  ...
  if(isLocked(email)){
   alert("You have already attempted the hackathon. Please try again after 15 days.");
   return;
  }
