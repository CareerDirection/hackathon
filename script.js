...
function resultKey(email){ return "hack_result_"+email; }
...
function saveResult(email,data){
  localStorage.setItem(resultKey(email),JSON.stringify(data));
}
function getSavedResult(email){
  return JSON.parse(localStorage.getItem(resultKey(email))||"null");
}
function clearLock(email){
  localStorage.removeItem(lockKey(email));
  localStorage.removeItem(resultKey(email));
}
...
$("adminResetBtn").addEventListener("click",()=>{
  ...
  if(code===ADMIN_RESET_CODE&&email){
    clearLock(email);
    alert("Lock & Result reset successful.");
...
$("startBtn").addEventListener("click",()=>{
  ...
  // 🔥 If result already exists → show result page
  const saved=getSavedResult(email);
  if(saved){
    displayResult(saved);
    return;
  }
...
function finishExam(){
  ...
  let rawScore=0,totalMarks=0,wrong=0,unanswered=0,correct=0;
  ...
  let finalScore=Math.max(0,Math.round((rawScore/totalMarks)*100));

  setLock(currentUser.email);

  const resultData={name:currentUser.name,email:currentUser.email,score:finalScore,correct,wrong,unanswered};
  saveResult(currentUser.email,resultData);
...
function displayResult(data){
  ...
  $("finalScore").textContent=`Final Score: ${data.score} / 100`;
  $("resultStudent").innerHTML=`${data.name} | ${data.email}`;
  $("resultBreakdown").innerHTML=`
    Attempted: ${QUESTIONS_PER_EXAM-data.unanswered}<br>
    Not Attempted: ${data.unanswered}<br>
    Correct: ${data.correct}<br>
    Wrong: ${data.wrong}<br>
    Negative Marks: ${data.wrong*WRONG_PENALTY}
  `;
}
