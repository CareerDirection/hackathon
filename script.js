...
const QUESTIONS_PER_EXAM = 2; // MODEL ONLY (you can increase later)
...
function issueWarning(msg) {
  ...
  beep();
  showWarningPopup(`Warning ${warnings}/${MAX_WARNINGS}: ${msg}`);
...
document.addEventListener("copy", e => { if (inExam) { e.preventDefault(); issueWarning("Copy attempt detected"); }});
document.addEventListener("contextmenu", e => { if (inExam) { e.preventDefault(); issueWarning("Right-click detected"); }});
document.addEventListener("mouseup", () => {
  if (inExam && window.getSelection().toString().length > 0) {
    issueWarning("Text selection detected");
...
function computeScore() {
  ...
  const final = Math.max(0, marks - wrongPenalty);
  return { correct, wrong, unanswered, wrongPenalty, final };
}
...
function finishExam() {
  ...
  $("finalScore").textContent = `Final Score: ${r.final} / 10`;
  $("resultBreakdown").innerHTML =
    `Attempted: ${r.correct + r.wrong}<br>
     Not Attempted: ${r.unanswered}<br>
     Correct: ${r.correct}<br>
     Wrong: ${r.wrong}<br>
     Negative Marks: -${r.wrongPenalty}`;

  $("resName").textContent = currentUser.name;
  $("resEmail").textContent = currentUser.email;
  $("resCollege").textContent = currentUser.college;
  $("resStream").textContent = currentUser.stream;

  setLock(currentUser.email);
  localStorage.setItem("last_attempt_email", currentUser.email);
  localStorage.setItem("last_attempt_result", JSON.stringify(r));
  localStorage.setItem("last_student_details", JSON.stringify(currentUser));
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const savedEmail = localStorage.getItem("last_attempt_email");
  const savedResult = localStorage.getItem("last_attempt_result");
  const savedStudent = localStorage.getItem("last_student_details");

  if (savedEmail && isLocked(savedEmail) && savedResult) {
    const s = JSON.parse(savedStudent);
    $("resName").textContent = s.name;
    $("resEmail").textContent = s.email;
    $("resCollege").textContent = s.college;
    $("resStream").textContent = s.stream;
    showSection("resultSection");
    showWarningPopup("You already attempted. Try again after 15 days.");
  }
  ...
  $("startBtn").onclick = () => {
    ...
    if (!name || !email || !college || !stream) {
      showWarningPopup("Please fill all fields");
      return;
    }
  ...
  $("adminResetBtn").onclick = () => {
    if ($("resetCode").value === ADMIN_RESET_CODE) {
      localStorage.clear();
      showWarningPopup("All locks reset.");
