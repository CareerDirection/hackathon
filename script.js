...
function showWarningPopup(message) {
  ...
  popup.textContent = `Warning ${warnings}/${MAX_WARNINGS}: ${message}`;
...
function handleVisibilityChange(){
 if(inExam && !quizFinished && document.hidden) issueWarning("Tab switch detected");
}
function handleWindowBlur(){
 if(inExam && !quizFinished) issueWarning("Window focus lost");
}
function handleCopy(e){
 if(inExam && !quizFinished){
  e.preventDefault();
  issueWarning("Copy attempt detected");
 }
}
function handleRightClick(e){
 if(inExam && !quizFinished){
  e.preventDefault();
  issueWarning("Right-click attempt detected");
 }
}
function handleTextSelection(){
 if(inExam && !quizFinished){
  ...
  if(selection.length > 0){
   issueWarning("Text selection attempt detected");
...
function resetAllLocks(){
 Object.keys(localStorage).forEach(k=>{if(k.startsWith("hack_lock_")) localStorage.removeItem(k);});
}

// ================= QUESTIONS =================
function shuffle(arr){arr.sort(()=>Math.random()-0.5);}
function pickQuestions(){
 const copy=[...questionBank]; shuffle(copy);
...
function updateStats(){
 const a=Object.keys(userAnswers).length;
 $("answeredCount").textContent=a;
 $("unansweredCount").textContent=chosenQuestions.length-a;
}
function renderAllQuestions(){
 ...
 chosenQuestions.forEach((q,i)=>{
  ...
  q.options.forEach(opt=>{
   const v=opt[0];
   div.innerHTML+=`<div class="optionRow"><label>
...
function computeScore(){
 ...
 if(unanswered > 10 && unanswered <= 15) unansweredPenalty = 3;
 else if(unanswered > 15 && unanswered <= 20) unansweredPenalty = 6;
 else if(unanswered > 20 && unanswered <= 25) unansweredPenalty = 9;
 else if(unanswered > 25) unansweredPenalty = 12;

 let final = marks - wrongPenalty - unansweredPenalty;
 if(final < 0) final = 0;
 return {correct, wrong, unanswered, wrongPenalty, unansweredPenalty, final};
}
...
function finishExam(){
 ...
 $("resultMessage").textContent="Thank you for participating! Keep learning and try again.";
...
document.addEventListener("DOMContentLoaded",()=>{
 $("startBtn").onclick=()=>{
  ...
  if(!name||!email||!college||!stream){showWarningPopup("Please fill all fields");return;}
  if(isLocked(email)){showWarningPopup("You already attempted. Try again after 15 days.");return;}
 ...
 $("adminResetBtn").onclick=()=>{
  if($("resetCode").value.trim()===ADMIN_RESET_CODE){
   resetAllLocks();
   showWarningPopup("All locks reset.");
   $("resetCode").value="";
  } else showWarningPopup("Invalid reset code");
 };
});
