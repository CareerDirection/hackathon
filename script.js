...
function renderSet(){
  ...
  const set=chosenQuestions.slice(start,start+10);
...
function computeScore(){
  ...
  chosenQuestions.forEach(q=>{
    ...
    if(!sel) unanswered++;
    else if(sel.value===q.correct){correct++;marks+=q.marks;}
    else wrong++;
  });
...
function finishExam(){
  ...
  $("resultBreakdown").innerHTML=`
    ...
    <ul class="violationList">
      <li>Correct Answers: ${r.correct} (+${r.marks})</li>
      <li>Wrong Answers: ${r.wrong} (-${r.wrongPenalty})</li>
      <li>Unanswered: ${r.unanswered} (-${r.unPenalty})</li>
    </ul>`;
...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  if(lastEmail && isLocked(lastEmail) && lastScore){
  ...
  $("startBtn").onclick=()=>{
    ...
    if(isLocked(email)){
      ...
      $("finalScore").textContent=`Your Last Score: ${localStorage.getItem("lastScore")} / 100`;
      showSection("resultSection");
      return;
    }
  ...
  $("nextSetBtn").onclick=()=>{
    if(currentSetIndex<2){currentSetIndex++;renderSet();}
    else finishExam();
  };
