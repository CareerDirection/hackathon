...
function pickQuestions(){const copy=[...questionBank];shuffle(copy);chosenQuestions=copy.slice(0,QUESTIONS_PER_EXAM);}
...
function finishExam(){
 ...
 $("resultStudent").innerHTML=`<b>Name:</b> ${currentUser.name}, <b>Email-ID:</b> ${currentUser.email}<br><b>College:</b> ${currentUser.college} | <b>Department:</b> ${currentUser.stream}`;
 $("finalScore").textContent=`Your Score: ${r.final} / 100`;
 $("resultBreakdown").innerHTML=`Attempted: ${r.correct+r.wrong}<br>Not Attempted: ${r.unanswered}<br>Correct: ${r.correct}<br>Wrong: ${r.wrong}<br>Negative Marks: -${r.wrongPenalty+r.unPenalty}`;
 $("resultMessage").textContent="Thank you for participating!";
 localStorage.setItem("lastScore",r.final);
 localStorage.setItem("lock_"+currentUser.email,Date.now()+LOCK_DAYS*86400000);
 showSection("resultSection");
}

document.addEventListener("DOMContentLoaded",()=>{
 $("startBtn").onclick=()=>{
  const name=$("fullName").value.trim(), email=$("email").value.trim(), college=$("college").value.trim(), stream=$("stream").value.trim();
  if(!name||!email||!college||!stream) return alert("Fill all fields");
  currentUser={name,email,college,stream};
  pickQuestions(); renderAllQuestions(); startCamera(); startTimer(); inExam=true;
  showSection("examSection");
 };
 $("nextSetBtn").onclick=finishExam;
 $("adminResetBtn").onclick=()=>{if($("resetCode").value===ADMIN_RESET_CODE){localStorage.clear();alert("Reset done");location.reload();}};
});
