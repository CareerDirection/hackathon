...
function issueWarning(reason) {
  ...
  if (r.includes("tab") || r.includes("window")) theme = "tab";
  else if (r.includes("sound") || r.includes("mic")) theme = "audio";
  else if (r.includes("camera")) theme = "camera";
  else if (
    r.includes("copy") ||
    r.includes("paste") ||
    r.includes("shortcut") ||
    r.includes("right-click") ||
    r.includes("selection")
  ) {
    theme = "keyboard";
  } else if (r.includes("developer tools") || r.includes("inspect") || r.includes("printscreen") || r.includes("screenshot")) {
    theme = "devtools";
  }
...
// ================== DEVICE ENFORCEMENT (MOBILE / TABLET BLOCK) ==================
function determineDeviceState() {
  ...
  // Treat small-screen, touch, or mobile UA as "mobile"
  if (isMobileUA || isTouch || isSmallScreen) return "mobile";
  return "desktop";
}

function setupDeviceEnforcement(startBtn, mobileWarningEl) {
  function applyHomeUI(state) {
    if (!startBtn || !mobileWarningEl) return;

    if (state === "mobile") {
      mobileWarningEl.classList.remove("hidden");
      startBtn.disabled = true;
      startBtn.style.opacity = "0.5";
      startBtn.style.cursor = "not-allowed";
    } else {
      mobileWarningEl.classList.add("hidden");
  ...
  function handleStateChange() {
    const newState = determineDeviceState();

    // Always apply UI for home page mode
    if (!inExam && !quizFinished) {
      applyHomeUI(newState);
    }

    // If in exam and state changes towards mobile-like, warn
    if (
      inExam &&
      !quizFinished &&
      ...
      issueWarning(
        "Window resized / device detection changed — continue only on Laptop/PC."
      );
    }
