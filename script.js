...
// =====================================================
// ✅ QUESTION BANK (ADD MORE QUESTIONS LATER)
// =====================================================
const questionBank = [
...
function pickQuestionsForExam(){
  ...
  chosenQuestions = shuffled.slice(0, 30); // up to 30 if available
}
...
function renderCurrentSet(){
  ...
  const questions = chosenQuestions.slice(start, start + 10);

  questions.forEach((q, idx) => {
    const div = document.createElement("div");
    div.innerHTML = `<p><b>Q${start + idx + 1}. ${q.text}</b></p>`;
...
function computeResult(){
  let correctMarks = 0, wrongCount = 0, unansweredCount = 0;

  chosenQuestions.forEach(q => {
    const sel = document.querySelector(`input[name="${q.id}"]:checked`);
    if(!sel) unansweredCount++;
    else if(sel.value === q.correct) correctMarks += q.marks;
    else wrongCount++;
  });
  ...
  return { correctMarks, wrongCount, unansweredCount, wrongPenalty, unPenalty, finalScore };
}
...
function finishQuiz(){
  ...
  $("resultBadgeBox").innerHTML = `
    <div class="violationBox">
      <p><b>Result Breakdown</b></p>
      Correct Marks: +${r.correctMarks}<br>
      Wrong Answers: ${r.wrongCount} (−${r.wrongPenalty})<br>
      Unanswered: ${r.unansweredCount} (−${r.unPenalty})<br>
    </div>`;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("nextSetBtn").onclick = () => {
    if(++currentSetIndex < 3) renderCurrentSet();
    else finishQuiz();
  };
