...
const LOCK_DAYS = 20;
const LOCK_TIME = LOCK_DAYS * 24 * 60 * 60 * 1000;
...
// ================= QUESTION BANK =================
// 🔹 PASTE YOUR 300 QUESTIONS BELOW THIS LINE
const questionBank = [
...
function addWarning(reason){
  ...
  if(warnings >= MAX_WARNINGS) finishQuiz();
}

// Tab switch
document.addEventListener("visibilitychange",()=>{ if(document.hidden) addWarning("Tab switched"); });

// Window blur
window.addEventListener("blur",()=> addWarning("Window lost focus"));

// DevTools detection
setInterval(()=>{
  const widthDiff = window.outerWidth - window.innerWidth;
  if(widthDiff > 160) addWarning("DevTools detected");
},2000);
...
function startTimer(){
  timerInterval = setInterval(()=>{
    ...
    if(remainingSeconds<=300) $("timer").classList.add("timerDanger");
...
function computeResults(){
  ...
  chosenQuestions.forEach(q=>{
    ...
    else if(sel.value===q.correct){correctMarks+=q.marks;correctCount++;}
    else wrongCount++;
  });

  const wrongPenalty=wrongCount*WRONG_PENALTY;
  const unPenalty=Math.floor(unansweredCount/5)*UNANSWERED_PENALTY_PER_5;
...
function finishQuiz(){
  ...
  localStorage.setItem("GLOBAL_EXAM_LOCK", Date.now());
...
document.addEventListener("DOMContentLoaded",()=>{
  const lastAttempt=localStorage.getItem("GLOBAL_EXAM_LOCK");
  if(lastAttempt && Date.now()-lastAttempt < LOCK_TIME){
    showSection("resultSection");
    return;
  }
  ...
  $("startBtn").onclick=()=>{
    ...
    chosenQuestions=chosenQuestions.slice(0,TOTAL_QUESTIONS_PER_EXAM);
    ...
    $("studentEmail").textContent=$("email").value;
    $("studentCollege").textContent=$("college").value;
    $("studentStream").textContent=$("stream").value;

    showSection("examSection");
  };

  $("nextSetBtn").onclick=finishQuiz;

  $("adminResetBtn").onclick=()=>{
    if($("resetCode").value===ADMIN_RESET_CODE){
      localStorage.removeItem("GLOBAL_EXAM_LOCK");
