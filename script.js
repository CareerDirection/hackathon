...
async function startCamera(){
  try{
    ...
    issueWarning("Camera access denied");
  }
}
...
function computeScore(){
  let correct=0,wrong=0,unans=0,marks=0;
  ...
  const unPenalty=Math.floor(unans/5)*UNANSWERED_BLOCK_PENALTY;
  let final=marks-penalty-unPenalty;
  if(final<0)final=0;
  return {correct,wrong,unans,marks,penalty,unPenalty,final};
}
...
function finishQuiz(){
  ...
  $("resultBreakdown").innerHTML=`
    ...
    <ul class="violationList">
      ...
      <li>Wrong Answers: ${r.wrong} (-${r.penalty})</li>
      <li>Unanswered: ${r.unans} (-${r.unPenalty})</li>
    </ul>`;
...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  $("startBtn").addEventListener("click",()=>{
    ...
    if(isLocked(email)){
      showSection("resultSection");
      $("finalScore").textContent="You already attempted. Try after 20 days.";
      return;
    }
