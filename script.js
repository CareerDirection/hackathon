...
function isDesktop() {
  ...
  const isMobile = /android|iphone|ipad|ipod|mobile|tablet|opera mini|iemobile|blackberry/.test(ua);
  const smallScreen = window.innerWidth < 1024;
  return !isMobile && !smallScreen;
}
...
if (!isDesktop()) {
  document.body.innerHTML = `
    <div style="font-family: Arial; text-align:center; padding:40px;">
      ...
      <p>This hackathon platform can only be accessed using a laptop or desktop computer.</p>
      <p>Please switch to a larger screen device and try again.</p>
    </div>`;
  throw new Error("Blocked non-desktop device");
}
...
function finishExam(){
 ...
 let msg="";
 if(r.final>=75) msg="🌟 Outstanding performance!";
 else if(r.final>=50) msg="🎉 Great job! Internship eligible.";
 else if(r.final>=30) msg="👍 Good attempt! Keep improving.";
 else msg="👏 Thank you for participating. Keep learning!";
...
document.addEventListener("DOMContentLoaded",()=>{
 ...
 $("adminResetBtn").onclick=()=>{
  if($("resetCode").value.trim()===ADMIN_RESET_CODE){
   resetAllLocks();
   alert("All locks have been reset.");
   $("resetCode").value="";
