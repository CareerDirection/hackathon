...
let chosenQuestions = [];
let userAnswers = {};
let remainingSeconds = EXAM_DURATION_MIN * 60;
let timerId = null;
let warnings = 0;
let inExam = false;
...
// ================= LOCK SYSTEM =================
function lockKey(email){return "hack_lock_"+email.toLowerCase();}
function isLocked(email){
  const exp=parseInt(localStorage.getItem(lockKey(email))||"0",10);
  return Date.now()<exp;
}
function setLock(email){
  const exp=Date.now()+LOCK_DAYS*24*60*60*1000;
  localStorage.setItem(lockKey(email),exp.toString());
}
...
function pickQuestions(){
  ...
  shuffle(copy);
  chosenQuestions=copy.slice(0,QUESTIONS_PER_EXAM);
}
...
function updateAnswerStats(){
  ...
  const total=chosenQuestions.length;
  $("answeredCount").textContent=answered;
  $("unansweredCount").textContent=total-answered;
}
...
function issueWarning(msg){
  ...
  beep();
  showToast(`Warning ${warnings}/${MAX_WARNINGS}: ${msg}`);
  if(warnings>=MAX_WARNINGS) finishExam();
}

function showToast(msg){
  const t=document.createElement("div");
  t.className="toastWarning default show";
  t.textContent=msg;
  document.body.appendChild(t);
  setTimeout(()=>t.remove(),3000);
}

document.addEventListener("visibilitychange",()=>{if(document.hidden)issueWarning("Tab switch");});
window.addEventListener("blur",()=>issueWarning("Window lost focus"));
...
function computeScore(){
  ...
  return {correct,wrong,unanswered,marks,wrongPenalty,unPenalty,final};
}
...
function finishExam(){
  ...
  const missed = r.unanswered;
  const negativeMarks = r.wrongPenalty + r.unPenalty;

  $("resultStudent").innerHTML =
    `<b>Name:</b> ${currentUser.name}, <b>Email-ID:</b> ${currentUser.email}<br>
     <b>College:</b> ${currentUser.college} | <b>Department:</b> ${currentUser.stream}`;

  $("finalScore").textContent=`Your Score: ${r.final} / 100`;

  $("resultBreakdown").innerHTML = `
    Total Questions: ${QUESTIONS_PER_EXAM}<br>
    Attempted: ${attempted}<br>
    Not Attempted: ${missed}<br>
    Correct Answers: ${r.correct}<br>
    Wrong Answers: ${r.wrong}<br>
    Negative Marks: -${negativeMarks}
  `;

  let msg="";
  if(r.final>=90) msg="Outstanding performance! Eligible for Paid Internship (Champion).";
  else if(r.final>=70) msg="Great job! Eligible for Paid Internship Interview (Winner).";
  else msg="Good attempt! Eligible for Unpaid Internship (Participation).";

  $("resultMessage").textContent=msg;

  localStorage.setItem("lastScore",r.final);
...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  $("startBtn").onclick=()=>{
    ...
    if(!name||!email||!college||!stream) return alert("Fill all fields");

    if(isLocked(email)){
      const lastScore=localStorage.getItem("lastScore")||"0";
      alert("You already attempted. Try again after 20 days.");
      $("finalScore").textContent=`Your Last Score: ${lastScore} / 100`;
      showSection("resultSection");
      return;
    }
